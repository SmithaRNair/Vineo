...
const BoxItem = ({ box }: { box: Box }) => (
  <div className="flex w-full flex-col items-center lg:w-4/5">
    <div className="box-item mb-4 ml-64 w-full rounded-lg border-2 border-gray-200 bg-white p-10 shadow-md">
      <div className="mb-4 text-left font-inter text-5xl text-logingray font-medium text-vineogray">
        {box.is_pending
      ...
      <div className="flex w-full flex-col items-start lg:w-full lg:flex-row">

        {/* Wine Box Container */}
        <div className="flex  w-full gap-6 rounded-lg border border-gray-200 bg-white p-8 shadow-md lg:w-2/3">
          {box.wines.map(wine => (
            <div key={wine.wine_id} className="w-full sm:w-1/2 lg:ml-5 lg:w-1/3 xl:w-1/4">
              <WineCard wine={wine} />
            </div>
          ))}
        </div>
        ...
        <div className="mt-6  flex w-full justify-end lg:ml-10 lg:mt-0 lg:w-1/3">
